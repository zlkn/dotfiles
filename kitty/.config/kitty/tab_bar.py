...
def draw_tab(
    ...
    deco_fg = as_rgb(0x5F87FF if tab.is_active else 0xA0A0A0)
    is_first = index == 1
    left_margin = min(UNDERLINE_MARGIN, left) if multi and is_first else 0
    right_margin = min(UNDERLINE_MARGIN, right) if multi and is_last else 0
    inner_left = left - left_margin
    inner_right = right - right_margin

    screen.cursor.decoration = 0
    screen.draw(" " * left_margin)

    screen.cursor.decoration = 1 if multi else 0
    screen.cursor.decoration_fg = deco_fg
    screen.draw(" " * inner_left + text + " " * inner_right)
